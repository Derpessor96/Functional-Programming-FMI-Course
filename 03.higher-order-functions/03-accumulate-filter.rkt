...
(define (accumulate-filter condition? operation null-value start end term next)
  (define (accumulate-filter-iterative current result-accumulator)
    (if (> current end)
        result-accumulator
        (if (condition? current)
            (accumulate-filter-iterative (next current) (operation (term current) result-accumulator))
            (accumulate-filter-iterative (next current) result-accumulator))))
  
  (accumulate-filter-iterative start null-value))
