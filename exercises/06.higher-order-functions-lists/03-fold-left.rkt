...
(define (fold-left op null-value xs)
  (define (helper xs result-accumulator)
    (if (null? xs)
        result-accumulator
        (helper (cdr xs) (op result-accumulator (car xs)))))

  (helper xs null-value))
...
(define (fold-left-rec op null-value xs)
  (define (helper xs)
    (if (null? xs)
        null-value
        (op (helper (cdr xs)) (car xs))))

  ; helper actually does a fold left on the reverse of its argument so we have to reverse it initially
  (helper (reverse xs)))
